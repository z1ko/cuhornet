...
Aatish, D. Bader, “Logarithmic Radix Binning and Vectorized Triangle Counting”,
...
TriangleCounting2::~TriangleCounting2() { release(); }

struct OPERATOR_InitTriangleCounts {
  triangle_t *d_triPerVertex;

  OPERATOR(Vertex &vertex) { d_triPerVertex[vertex.id()] = 0; }
};
...
struct OPERATOR_AdjIntersectionCountBalanced {
  ...
  OPERATOR(Vertex &u, Vertex &v, vid_t *ui_begin, vid_t *ui_end,
           vid_t *vi_begin, vid_t *vi_end, int FLAG) {
    int count = 0;
    if (!FLAG) {
...
triangle_t TriangleCounting2::countTriangles() {
  ...
  // triangle_t sum=gpu::reduce(hd_triangleData().triPerVertex,
  // hd_triangleData().nv+1);
