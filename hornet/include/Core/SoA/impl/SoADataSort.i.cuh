...
template <typename T> __device__ void bubble_sort(int32_t size, T *edges) {
  T temp;
...
__launch_bounds__(32) __global__
    ...
    small_segmented_sort_kernel(CSoAPtr<EdgeTypes...> edges,
                                degree_t *global_index, const degree_t nE,
...
__launch_bounds__(32) __global__
    typename std::enable_if<(2 == sizeof...(EdgeTypes)), void>::type
    small_segmented_sort_kernel(CSoAPtr<EdgeTypes...> edges, const degree_t nE,
                                const degree_t segment_length) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename degree_t, typename... EdgeTypes>
typename std::enable_if<(2 >= sizeof...(EdgeTypes)), void>::type
small_segmented_sort(CSoAPtr<EdgeTypes...> soa, degree_t capacity,
                     degree_t segment_length) {
  detail::small_segmented_sort_kernel<<<capacity / (32 * segment_length), 32>>>(
      soa, capacity, segment_length);
  CHECK_CUDA_ERROR
}
...
small_segmented_sort(CSoAPtr<EdgeTypes...> &soa, degree_t capacity,
  ...
  detail::small_segmented_sort_kernel<<<capacity / (32 * segment_length), 32>>>(
      temp_soa, index.data().get(), capacity, segment_length);
  // TODO : Check correctness
  RecursiveCopy<0, 0>::copy(temp_soa, DeviceType::DEVICE, soa,
                            DeviceType::DEVICE, capacity);
  RecursiveGather<1, sizeof...(EdgeTypes)>::assign(temp_soa, soa, index,
                                                   capacity);
}

template <typename T, int BLOCK_THREADS, int ITEMS_PER_THREAD>
__launch_bounds__(BLOCK_THREADS) __global__ void CubBlockSortKernel(T *edges) {
  using namespace cub;
  ...
  typedef BlockLoad<T, BLOCK_THREADS, ITEMS_PER_THREAD,
...
template <typename T0, typename T1, int BLOCK_THREADS, int ITEMS_PER_THREAD>
__launch_bounds__(BLOCK_THREADS) __global__
    void CubBlockSortKernel(T0 *edge_keys, T1 *edge_vals) {
  using namespace cub;
  // Specialize BlockLoad type for our thread block (uses warp-striped loads for
  // coalescing, then transposes in shared memory to a blocked arrangement)
  typedef BlockLoad<T0, BLOCK_THREADS, ITEMS_PER_THREAD,
                    BLOCK_LOAD_WARP_TRANSPOSE>
      BlockLoadT0;
  typedef BlockLoad<T1, BLOCK_THREADS, ITEMS_PER_THREAD,
                    BLOCK_LOAD_WARP_TRANSPOSE>
      BlockLoadT1;
  // Specialize BlockRadixSort type for our thread block
  typedef BlockRadixSort<T0, BLOCK_THREADS, ITEMS_PER_THREAD, T1>
      BlockRadixSortT;
  // Shared memory
  __shared__ union TempStorage {
...
template <typename degree_t, typename... EdgeTypes>
typename std::enable_if<(1 == sizeof...(EdgeTypes)), void>::type
cub_block_segmented_sort(CSoAPtr<EdgeTypes...> soa, degree_t capacity,
                         degree_t segment_length) {
  using T = typename xlib::SelectType<0, EdgeTypes...>::type;
  T *edges = soa.template get<0>();
  int blocks = capacity / segment_length;
  if (segment_length == 32)
    CubBlockSortKernel<T, 32, 1><<<blocks, 32>>>(edges);
  if (segment_length == 64)
    CubBlockSortKernel<T, 32, 2><<<blocks, 32>>>(edges);
  if (segment_length == 128)
    CubBlockSortKernel<T, 32, 4><<<blocks, 32>>>(edges);
  if (segment_length == 256)
    CubBlockSortKernel<T, 32, 8><<<blocks, 32>>>(edges);
  if (segment_length == 512)
    CubBlockSortKernel<T, 32, 16><<<blocks, 32>>>(edges);
  if (segment_length == 1024)
    CubBlockSortKernel<T, 256, 4><<<blocks, 256>>>(edges);
  if (segment_length == 2048)
    CubBlockSortKernel<T, 256, 8><<<blocks, 256>>>(edges);
  if (segment_length == 4096)
    CubBlockSortKernel<T, 256, 16><<<blocks, 256>>>(edges);
}
...
cub_block_segmented_sort(CSoAPtr<EdgeTypes...> soa, degree_t capacity,
  ...
  int blocks = capacity / segment_length;
  if (segment_length == 32)
    CubBlockSortKernel<T0, T1, 32, 1><<<blocks, 32>>>(key, val);
  if (segment_length == 64)
    CubBlockSortKernel<T0, T1, 32, 2><<<blocks, 32>>>(key, val);
  if (segment_length == 128)
    CubBlockSortKernel<T0, T1, 32, 4><<<blocks, 32>>>(key, val);
  if (segment_length == 256)
    CubBlockSortKernel<T0, T1, 32, 8><<<blocks, 32>>>(key, val);
  if (segment_length == 512)
    CubBlockSortKernel<T0, T1, 32, 16><<<blocks, 32>>>(key, val);
  if (segment_length == 1024)
    CubBlockSortKernel<T0, T1, 256, 4><<<blocks, 256>>>(key, val);
  if (segment_length == 2048)
    CubBlockSortKernel<T0, T1, 256, 8><<<blocks, 256>>>(key, val);
  if (segment_length == 4096)
    CubBlockSortKernel<T0, T1, 256, 16><<<blocks, 256>>>(key, val);
}
...
template <typename degree_t, typename... EdgeTypes>
typename std::enable_if<(1 == sizeof...(EdgeTypes)), void>::type
cub_segmented_sort(CSoAPtr<EdgeTypes...> &soa, degree_t capacity,
                   degree_t segment_length) {
  CSoAData<TypeList<EdgeTypes...>, DeviceType::DEVICE> temp_data(capacity);
  temp_data.copy(soa, DeviceType::DEVICE, capacity);
  CSoAPtr<EdgeTypes...> temp_soa = temp_data.get_soa_ptr();
  ...
  thrust::transform(rmm::exec_policy(stream), offsets.begin(), offsets.end(),
                    thrust::make_constant_iterator(segment_length),
                    offsets.begin(), thrust::multiplies<degree_t>());

  size_t tempStorageBytes = 0;

  cub::DeviceSegmentedRadixSort::SortKeys(
      NULL, tempStorageBytes, in_edges, out_edges, capacity, offsets.size() - 1,
      offsets.data().get(), offsets.data().get() + 1);
...
cub_segmented_sort(CSoAPtr<EdgeTypes...> &soa, degree_t capacity,
  ...
  rmm::device_buffer tempStorage(tempStorageBytes, rmm::cuda_stream_view{});
  cub::DeviceSegmentedRadixSort::SortPairs(
      tempStorage.data(), tempStorageBytes, in_key, out_key, in_val, out_val,
      capacity, offsets.size() - 1, offsets.data().get(),
      offsets.data().get() + 1);
}
...
cub_segmented_sort(CSoAPtr<EdgeTypes...> &soa, degree_t capacity,
  ...
  RecursiveGather<1, sizeof...(EdgeTypes)>::assign(temp_soa, soa, out_index,
                                                   capacity);
}
...
void CSoAData<TypeList<Ts...>, device_t>::segmented_sort(
    int segment_length_log2) {
  // static_assert(device_t == DeviceType::DEVICE, "CSoAData<TypeList<Ts...>,
  // device_t>::segmented_sort called with device_t != DeviceType::DEVICE");
